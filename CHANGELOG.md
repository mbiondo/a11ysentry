...
### Fixed
- **MCP Server**: Fixed massive token bloat by applying global deduplication to violations returned by the `analyze_accessibility` tool. Violations from components used across multiple pages are now correctly deduplicated before returning the result to the LLM.
...
### Fixed
- **MCP Registration**: Refactored `mcp --register` to merge configuration into a dynamic map, preventing destructive overwriting of existing user settings (like UI themes or other agent configurations) when registering the A11ySentry MCP server.
